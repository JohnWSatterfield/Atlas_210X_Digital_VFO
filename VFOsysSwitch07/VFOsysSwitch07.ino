 ...
 * 
 * changes needed for 2.2 KHZ Drake Filter
 * 
...
#define IF 5643500               // IF amplifier frequency 5645000
...
#define VERSION "Ver. 2.07"
#define ID "by KI5IDZ"

#define CORRECTION 70780ULL  
//tuned on 20m at 14.250 - 
//Correction 70780ULL 8606400 with TR-7 crystal filter & Si5351 carrier oscillator 
//Correction 70780ULL 8605000 with Atlas crystal filter with stock or Si5351 carrier oscillator
...
long freq = 14235000;
long ifFreq = 5643500;  //5645000;
long oppFreq = 5646500; //5648300;
long cwFreq = 5644700; //5646200;
int32_t calibA=0;
long fstep = 1000;
long interfreq = 0;
long encodercount = 0;
...
//------------------------------------------------------------------------
    ...
    //rtc.adjust(DateTime(2022, 7, 17, 1, 07, 10));
...
/*void serialprintcount1(int32_t &quad,long &freq) { // code is for an encoder with i2cEncoder
  readencoder();
  if (quad != 0) {
    if (quad >= EncoderStep || quad <= -EncoderStep){
     if (quad >  EncoderStep) quad =  EncoderStep;
     ...
     freq = freq + long((quad * fstep)/EncoderStep);
     set_frequency();
     Serial.print("quad: "); Serial.print(quad); Serial.print(" count: ");
     Serial.print(encodercount); Serial.print(" Freq: "); Serial.println(freq);
     quad = 0;
     f_dchange=1;
    }
  }
...
void CalibrateIs5351() {
  int32_t freqcalib = freq;
  long calibAA = 0;
  ...
  while (calibrating) {
    serialprintcalib();
    calibAOFF = digitalRead(button2);
    delay(50);
    if (!calibAOFF) calibrating = false;
  }
  calibAA = (freqcalib - freq)*53;
  calibAA = (CORRECTION-calibAA);
  calibA = calibAA;
  si5351.set_correction(calibA,SI5351_PLL_INPUT_XO);
  Serial.print("old correction = "); Serial.println(CORRECTION); 
  Serial.print("new correction = "); Serial.println(calibA);
  freq = freqcalib;
  setstep1();
  f_dchange=1;
  delay(500);
...
void bandlist() {                //procedure for displaying the correct band frequency & calibration of each band
 ...
 /* if (count == 3) interfreq= IF - 0;       //count = 3 (WWV on 40m) + or - as needed to calibrate radio */
  if (count == 3) interfreq = -IF + 0;       //count = 3 (20m) IF Frequency for USB starting with 20m (8605000)
  if (count == 4) interfreq = -IF + 0;       //count = 4 (15) on the display screen
  if (count == 5) interfreq = -IF + 0;       //count = 5 (10m) on the display screen
...
void display_write() {
  char str[64];    
    if(f_dchange==1){     //check if display change variable is true then enter this section of code
        f_dchange=0;        
        //GRAM_clr();  //clears the screen
        boxfill(0,0,Nx-1,Ny-1,cl_BG);     
        //---------------- Display Dial ------------------------------------------
        Dial(freq); //sends the frequency to the dial subroutine
